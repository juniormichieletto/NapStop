...
> 2. **Continuous Verification:** You MUST run the tests (`gradle :app:testDebugUnitTest`) and they must be passing after each change. Also compile the applet with `gradle :app:assembleDebug` or `compile_applet`.
